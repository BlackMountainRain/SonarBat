...
type HostQuery struct {
	...
	withHostBlacklist *HostBlacklistQuery
...
// QueryHostBlacklist chains the current query on the "host_blacklist" edge.
func (hq *HostQuery) QueryHostBlacklist() *HostBlacklistQuery {
	query := (&HostBlacklistClient{config: hq.config}).Query()
...
func (hq *HostQuery) Clone() *HostQuery {
	...
	return &HostQuery{
		...
		withHostBlacklist: hq.withHostBlacklist.Clone(),
...
// WithHostBlacklist tells the query-builder to eager-load the nodes that are connected to
// the "host_blacklist" edge. The optional arguments are used to configure the query builder of the edge.
func (hq *HostQuery) WithHostBlacklist(opts ...func(*HostBlacklistQuery)) *HostQuery {
	query := (&HostBlacklistClient{config: hq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	hq.withHostBlacklist = query
	return hq
}
...
func (hq *HostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Host, error) {
	...
	if query := hq.withHostBlacklist; query != nil {
		if err := hq.loadHostBlacklist(ctx, query, nodes,
			func(n *Host) { n.Edges.HostBlacklist = []*HostBlacklist{} },
			func(n *Host, e *HostBlacklist) { n.Edges.HostBlacklist = append(n.Edges.HostBlacklist, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (hq *HostQuery) loadHostBlacklist(ctx context.Context, query *HostBlacklistQuery, nodes []*Host, init func(*Host), assign func(*Host, *HostBlacklist)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(hostblacklist.FieldHostID)
	}
	query.Where(predicate.HostBlacklist(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(host.HostBlacklistColumn), fks...))
	}))
